...
/* USER CODE BEGIN PV */
volatile bool medicion_completa = false;
uint32_t num_grey;
int array[16];
/* USER CODE END PV */
...
int main(void)
{
  ...
  for (int i = 0; i < 16; i++) {
       ...
       while (!medicion_completa) {
                   HAL_Delay(5);  // Pequeño retraso para no bloquear el sistema
       }
       medicion_completa = false;
       //HAL_Delay(3000); // Esperar 1 segundo entre cambios
       }
  /* Infinite loop */
